...
describe Stretchy::Queries::MoreLikeThisQuery do
  ...
  describe 'initializes with' do
    specify 'like_text' do
      expect(described_class.new(like_text: 'this is a string').like_text).to eq('this is a string')
    end

    specify 'docs' do
      instance = described_class.new(docs: docs)
      expect(instance.docs.first).to eq('_index' => SPEC_INDEX, '_type' => FIXTURE_TYPE, '_id' => 1)
    end

    specify 'ids' do
      instance = described_class.new(ids: [1,2])
  ...
  describe 'produces json for' do
    specify 'like_text' do
      json = described_class.new(like_text: 'this is a string').to_search[:more_like_this]
      expect(json[:like_text]).to eq('this is a string')
    end

    specify 'docs' do
      json = described_class.new(docs: docs).to_search[:more_like_this]
    ...
    specify 'ids' do
      json = described_class.new(ids: [1,2]).to_search[:more_like_this]
    ...
    specify 'attributes' do
      ...
      expect(json[:fields]).to eq(fields.map(&:to_s))
