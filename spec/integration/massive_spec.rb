...
describe 'a massive query' do
  ...
  it 'is much less typing with Stretchy' do
    ...
    expect(query).to include(match: { '_all' => { query: 'all_match' }})
    expect(query).to include(match: { 'match_field' => { query: 'match_field_string' }})
    expect(query).to include(match: { 'must_string_field' => { query: 'must_string' }})
    ...
    expect(not_query).to include(match: {'_all' => { query: 'all_not_match'}})
    expect(not_query).to include(match: {'not_match_field' => { query: 'not_match_field_string'}})
    expect(not_query).to include(match: {'must_not_string_field' => { query: 'must_not_string' }})
